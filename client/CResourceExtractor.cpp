...
bool parse_folders = 0; // enable disable the whole parsing thing. (Use only if you know what this is doing. krs)
...
bool pcx_to_bmp = 1; // converts .pcx to bmp. Can be used when you have .pcx converted already (Use only if you know what this is doing. krs)
...
void splitDefFile(std::string fileName, std::string spritesPath)
{
	if (CResourceHandler::get()->existsResource(ResourceID("SPRITES/" + fileName)))
	{
		...
		{
			std::string newFilename = spritesPath + removeExtension(fileName) + boost::lexical_cast<std::string>(i) + ".bmp";
			SDL_SaveBMP(cde->ourImages[i].bitmap, newFilename.c_str());
		}
...
void moveFile(std::string sourceFilePath, std::string destinationFilePath)
{
...
void moveFile(std::string filename, std::string sourceFolder, std::string destinationFolder)
{
	std::string sourceFilePath = sourceFolder + filename;
	if(boost::filesystem::exists(sourceFilePath))
	{
		if(only_copy_files)
...
void moveFileFromConfig(const JsonNode node, std::string firstNode, std::string secondNode,
						std::string sourceRoot, std::string destinationRoot)
{
...
void parseFolders()
{
	...
	std::string modPath = VCMIDirs::get().userDataPath() + "/ModsTemp/";
	std::string dataPath = VCMIDirs::get().userDataPath() + "/Data/Temp/";
	std::string spritesPath = dataPath + "Sprites/";
	std::string imagesPath = dataPath + "Images/";
	std::string soundPath = dataPath + "Sound/";
	std::string videoPath = dataPath + "Video/";
	std::string mp3Path = VCMIDirs::get().userDataPath() + "/Mp3/";
	...
	destinationPath = modPath + "SoD/mods/artifacts/resources/sprites/SoD/";
	for(int i=1; i<=144; i++)
	{
	...
	destinationPath = modPath + "SoD/mods/creatureBanks/resources/sprites/SoD/";
	...
	//for(const JsonNode &spell : configSpellInfo["spells"].Struct())
	...
	for(std::string faction: factions)
	{
		...
		const JsonNode configCreatureList(ResourceID("ModsTemp/SoD/mods/factions/mods/" + faction + "/mod.json"));
		destinationPath = modPath + "SoD/mods/factions/mods/" + faction + "/resources/";
		for(const JsonNode &creatureMod : configCreatureList["creatures"].Vector())
		{
			...
			const JsonNode configCreatures(ResourceID("ModsTemp/SoD/mods/factions/mods/"+ faction + "/resources/" + configFilePath));
		...
		destinationPath = modPath + "SoD/mods/factions/mods/" + faction + "/resources/";
		const JsonNode configHeroesList(ResourceID("ModsTemp/SoD/mods/factions/mods/" + faction + "/mod.json")); // list of config files
		...
		for(const JsonNode &heroClasesNode : configHeroesList["heroClasses"].Vector())
		{
			...
			const JsonNode configHeroClass(ResourceID("ModsTemp/SoD/mods/factions/mods/" + faction + "/resources/config/heroClasses/" + configFileName));
		...
		for(const JsonNode &heroNode : configHeroesList["heroes"].Vector()) // list of config files
		{
			...
			const JsonNode configHeroes(ResourceID("ModsTemp/SoD/mods/factions/mods/" + faction + "/resources/config/heroes/" + configFileName));
		...
		destinationPath = modPath + "SoD/mods/factions/mods/" + faction + "/resources/";
		const JsonNode configDwellings(ResourceID("ModsTemp/SoD/mods/factions/mods/"+ faction + "/resources/config/factions/" + faction + "/dwellings.json"));
		...
		const JsonNode configFaction(ResourceID("ModsTemp/SoD/mods/factions/mods/" + faction + "/resources/config/factions/" + faction + "/faction.json"));
		...
		// move puzzle files
		destinationPath = modPath + "SoD/mods/factions/mods/" + faction + "/resources/sprites/factions/" + faction + "/puzzleMap/";
		const JsonNode configPuzzle(ResourceID("ModsTemp/SoD/mods/factions/mods/" + faction + "/resources/config/factions/" + faction +  "/puzzleMap.json"));
		...
		// move siege files
		destinationPath = modPath + "SoD/mods/factions/mods/" + faction + "/resources/sprites/factions/" + faction + "/siege/";
		const JsonNode configSiege(ResourceID("ModsTemp/SoD/mods/factions/mods/" + faction + "/resources/config/factions/" + faction + "/town/siege.json"));
		...
		destinationPath = modPath + "SoD/mods/factions/mods/" + faction + "/resources/sprites/factions/" + faction + "/townScreen/";
		const JsonNode configStructures(ResourceID("ModsTemp/SoD/mods/factions/mods/" + faction + "/resources/config/factions/" + faction + "/town/structures.json"));
		...
		const JsonNode configTown(ResourceID("ModsTemp/SoD/mods/factions/mods/" + faction + "/resources/config/factions/" + faction + "/town/town.json"));
		destinationPath = modPath + "SoD/mods/factions/mods/" + faction + "/resources/";
